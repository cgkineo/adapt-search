...
const searchDefaults = { // override in course.json "_search": {}
...
class Search extends Backbone.Controller {
  ...
  initialize() {
    this.model = null;
    this._searchableModels = null;
    this._wordIndex = null;
    this._regularExpressions = {
  ...
  setupConfig() {
    ...
    const modelWithDefaults = _.extend(searchDefaults, model);
  ...
  collectModelTexts() {
    const searchAttributes = this.model.get('_searchAttributes');
    const hideComponents = this.model.get('_hideComponents');
    const hideTypes = this.model.get('_hideTypes');
    const regularExpressions = this._regularExpressions;
    ...
    function combineAdaptModels() {
      ...
      const filtered = _.filter(rtn, function (model) {
    ...
    function getSearchableModels() {
      const adaptModels = combineAdaptModels();

      const searchable = [];
      for (let i = 0, l = adaptModels.length; i < l; i++) {
        const model = adaptModels[i];
    ...
    function recursivelyCollectModelTexts(json, level) {
      let texts = [];
      for (let i = 0, l = searchAttributes.length; i < l; i++) {
        const attributeObject = searchAttributes[i];
        ...
        switch (typeof json[attributeObject._attributeName]) {
          case 'object':
            if (json[attributeObject._attributeName] instanceof Array) {
              for (let sa = 0, sal = json[attributeObject._attributeName].length; sa < sal; sa++) {
      ...
      function addString(string, level, attributeObject) {
        const textLevel = level || attributeObject._level;
        let text = $('<div>' + string.replace(regularExpressions.trimReplaceWhitespace, '') + '</div>').text();
  ...
  makeModelTextProfiles() {
    // Handle _ignoreWords as a special case to support the authoring tool
    const ignoreWords = this.model.get('_ignoreWords') instanceof Array ?
      this.model.get('_ignoreWords') :
      this.model.get('_ignoreWords').split(',');

    const regularExpressions = this._regularExpressions;
    const searchAttributes = this.model.get('_searchAttributes');
    const minimumWordLength = this.model.get('_minimumWordLength');

    let scores = _.uniq(_.pluck(searchAttributes, '_level'));
    scores = _.map(scores, function (l) { return 1 / l; });

    for (let i = 0, l = this._searchableModels.models.length; i < l; i++) {
      const item = this._searchableModels.models[i];
      const profile = item.get('_searchProfile');
    ...
    function makeModelPhraseProfile(profile) {
      profile._phrases = [];
      const phrases = _.groupBy(profile._raw, function (phrase) {
        return phrase.text;
      });
      for (const p in phrases) {
        const bestItem = _.max(phrases[p], function (item) {
          return item.score;
        });
    ...
    function makeModelPhraseWordAndWordProfile(profile) {
      profile._words = [];
      for (let l = 0, ll = scores.length; l < ll; l++) {
        const score = scores[l];
        const phrases = _.where(profile._phrases, { score: score });
        let scoreWords = [];
        for (let i = 0, pl = phrases.length; i < pl; i++) {
          const phraseObject = phrases[i];
          const chunks = phraseObject.phrase.match(regularExpressions.matchNotWordBoundaries);
          const words = _.map(chunks, function (chunk) {
  ...
  indexTextProfiles() {
    ...
    for (let i = 0, il = this._searchableModels.models.length; i < il; i++) {
      const item = this._searchableModels.models[i];
      const id = item.get('_id');
      const searchProfile = item.get('_searchProfile');

      for (let w = 0, wl = searchProfile._words.length; w < wl; w++) {
        const word = searchProfile._words[w].word;
        if (Object.prototype.hasOwnProperty && !Object.prototype.hasOwnProperty.call(this._wordIndex, word)) {
    ...
    for (const _word in this._wordIndex) {
  ...
  find(findPhrase) {
    ...
    */
    ...
    const regularExpressions = this._regularExpressions;
    const wordIndex = this._wordIndex;
    // Handle _ignoreWords as a special case to support the authoring tool
    const ignoreWords = this.model.get('_ignoreWords') instanceof Array ?
      this.model.get('_ignoreWords') :
      this.model.get('_ignoreWords').split(',');
    const scoreQualificationThreshold = this.model.get('_scoreQualificationThreshold');
    const minimumWordLength = this.model.get('_minimumWordLength');
    const frequencyImportance = this.model.get('_frequencyImportance');
    const matchOn = this.model.get('_matchOn') || {};

    const json = this._searchableModels.toJSON();

    function getFindPhraseWords(findPhrase) {
      const findChunks = findPhrase.match(regularExpressions.matchNotWordBoundaries);
      let findWords = _.map(findChunks, function (chunk) {
        return chunk.replace(regularExpressions.trimReplaceNonWordCharacters, '');
      });
    ...
    function getMatchingIdScoreObjects(findWords) {
      ...
      for (const findWord in findWords) {
        for (const indexWord in wordIndex) {
          ...
          const rIndexWordBegins = new RegExp('^' + regularExpressions.escapeRegExp(indexWord), 'g');
          ...
          const rFindWordContains = new RegExp(regularExpressions.escapeRegExp(findWord), 'g');
          // allow only start matches on indexWord beginning with findWord i.e. find: one begins index: oneness
          const rFindWordBegins = new RegExp('^' + regularExpressions.escapeRegExp(findWord), 'g');
    ...
    function updateIdScoreObjectsForWord(matchingIdScoreObjects, word, isFullMatch, partMatchRatio) {
      ...
      for (let i = 0, l = wordIndex[word].length; i < l; i++) {
        const id = wordIndex[word][i];
        const model = _.findWhere(json, { _id: id });
        ...
        const allPhraseWordRatingObjects = _.where(model._searchProfile._words, { word: word });
    ...
    function filterAndSortQualifyingMatches(matchingIdScoreObjects) {
      const allowedScoreObjects = _.filter(matchingIdScoreObjects, function (item) {
        return isModelSearchable(item.model);
      });
      const qualifyingScoreThreshold = 1 / scoreQualificationThreshold;
      let qualifyingMatches = _.filter(allowedScoreObjects, function (item) {
    ...
    function mapSearchPhrasesToMatchingWords(matchingIdScoreObjects) {
      for (let i = 0, l = matchingIdScoreObjects.length; i < l; i++) {
        const matchingPhrases = [];
        const scoreObject = matchingIdScoreObjects[i];
        const foundWords = _.keys(scoreObject.foundWords);
        const modelPhrases = scoreObject.model.get('_searchProfile')._phrases;
        for (let p = 0, lp = modelPhrases.length; p < lp; p++) {
          const modelPhrase = modelPhrases[p];
    ...
    const findWords = getFindPhraseWords(findPhrase);
    const matchingIdScoreObjects = getMatchingIdScoreObjects(findWords);
    mapSearchPhrasesToMatchingWords(matchingIdScoreObjects);
    const qualifyingMatchedScoreObjects = filterAndSortQualifyingMatches(matchingIdScoreObjects);
...
export default (window.search = new Search());
